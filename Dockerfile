...
# fetch stats from a local endpoint
    (crontab -l 2>/dev/null; echo "0 3 * * * curl -s localhost:8888/stats -o ${CONFIG_PATH}/stats/$(date +%d.%m.%y).log >> /var/log/cron.log 2>&1") | crontab - ;\
...
# healthcheck
HEALTHCHECK --interval=180s --timeout=30s --start-period=10s CMD curl -f http://localhost:8888/stats || exit 1
